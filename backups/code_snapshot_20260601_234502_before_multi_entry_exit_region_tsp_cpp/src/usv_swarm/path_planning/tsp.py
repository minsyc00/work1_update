...
def solve_single_usv_tsp_cpp(
    ...
    if experiment_record is not None:
        ...
        experiment_record["pattern_selection"] = _pattern_selection_report(best_order, start_pose, graph, config)
    ...
    while remaining:
        ...
        for path_region in astar_path:
            ...
            best_pattern = min(
                ...
                key=lambda pattern: _transition_length(current_pose, pattern.entry_pose, config) + pattern.estimated_time,
            )
...
def _evaluate_order(
    ...
    for idx, region_id in enumerate(order):
        ...
        pattern = _select_pattern(region_id, current_pose, next_region, graph, config)
...
def _pattern_selection_report(
    ...
    for idx, region_id in enumerate(order):
        ...
        for pattern in candidates:
            ...
            total = inside + connect + collision + lookahead + infeasible
    ...
    while remaining:
        ...
        for idx, region_id in enumerate(remaining):
            ...
            if region_segments is None:
                continue
            accepted_index = idx
            accepted_segments = region_segments
            accepted_region_id = region_id
            accepted_pattern = pattern
            break
        if accepted_segments is None or accepted_pattern is None:
            break

...
def evaluate_tour_coverage_residuals(
    ...
    return evaluate_tour_coverage_state(config, tours, resolution, obstacle_field).residual_components
...
def evaluate_tour_coverage_state(
    ...
    for tour in tours:
        ...
        for segment in tour.segments:
            if segment.kind == "cover" and len(segment.waypoints) >= 2:
                start = segment.waypoints[0]
                end = segment.waypoints[-1]
                pass_segments.append((Pose2D(start.x, start.y, start.psi), Pose2D(end.x, end.y, end.psi)))

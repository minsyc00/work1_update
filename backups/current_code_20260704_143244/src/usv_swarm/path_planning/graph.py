...
from .types import DecomposedRegion, ObstacleField, PathPlanningConfig, RegionCoveragePattern, RegionGraph
...
def build_region_graph(
    ...
    for region_id, candidates in patterns.items():
        if not candidates:
            continue
        best = _best_workload_pattern(candidates, path_config, config)
        node_weights[region_id] = _pattern_workload_weight(best, path_config, config)
    ...
    for idx, region_a in enumerate(region_items):
        for region_b in region_items[idx + 1 :]:
            if region_b.region_id in region_a.neighbors or _bounds_touch_or_near(region_a.bounds, region_b.bounds):
                ...
                weight, meta = _region_transition_weight(region_a, region_b, patterns, config, obstacle_field, path_config)
...
def _region_transition_weight(
    ...
    if candidates_a and candidates_b:
        best_a = _best_workload_pattern(candidates_a, path_config, config)
        best_b = _best_workload_pattern(candidates_b, path_config, config)
...
def _best_workload_pattern(
    candidates: List[RegionCoveragePattern],
    path_config: PathPlanningConfig | None,
    config: PlannerConfig,
) -> RegionCoveragePattern:
    feasible = [candidate for candidate in candidates if candidate.feasible] or list(candidates)
    if not feasible:
        raise ValueError("expected at least one region coverage pattern")
    if not _coverage_priority_enabled(config, path_config):
        return min(feasible, key=lambda item: item.estimated_time)
    target = 0.0 if path_config is None else max(0.0, min(1.0, path_config.min_sweep_pattern_coverage_fraction))
    eligible = [candidate for candidate in feasible if _pattern_coverage_fraction(candidate) + 1e-9 >= target]
    pool = eligible or sorted(feasible, key=lambda item: (-_pattern_coverage_fraction(item), item.estimated_time))[:1]
    return min(pool, key=lambda item: _pattern_workload_weight(item, path_config, config))


def _pattern_workload_weight(
    pattern: RegionCoveragePattern,
    path_config: PathPlanningConfig | None,
    config: PlannerConfig,
) -> float:
    if not _coverage_priority_enabled(config, path_config):
        return pattern.estimated_time
    target = 0.0 if path_config is None else max(0.0, min(1.0, path_config.target_coverage_fraction))
    weight = 0.0 if path_config is None else max(path_config.coverage_priority_weight, 0.0)
    coverage_deficit = max(0.0, target - _pattern_coverage_fraction(pattern))
    return pattern.estimated_time + weight * coverage_deficit


def _coverage_priority_enabled(config: PlannerConfig, path_config: PathPlanningConfig | None) -> bool:
...
def _pattern_coverage_fraction(pattern: RegionCoveragePattern) -> float:
    try:

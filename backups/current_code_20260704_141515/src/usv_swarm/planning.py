...
@dataclass
class _ConflictSearchResult:
    conflict: Optional[_Conflict]
    checks: int
...
def _find_first_conflict_indexed(reservations: Dict[int, List[ReservationEntry]]) -> _ConflictSearchResult:
    earliest: Optional[_Conflict] = None
    checks = 0

    by_resource: Dict[str, List[ReservationEntry]] = {}
    by_edge: Dict[Tuple[str, str], List[ReservationEntry]] = {}
    for entry in itertools.chain.from_iterable(reservations.values()):
        by_resource.setdefault(entry.resource_id, []).append(entry)
        by_edge.setdefault((entry.from_node, entry.to_node), []).append(entry)

    for entries in by_resource.values():
        ordered = sorted(entries, key=lambda item: item.t_enter)
        for first_index, a in enumerate(ordered):
            for b in ordered[first_index + 1 :]:
                if b.t_enter >= a.t_exit:
                    break
                checks += 1
                if a.agent_id == b.agent_id or not _overlaps(a.t_enter, a.t_exit, b.t_enter, b.t_exit):
                    continue
                candidate = _make_conflict("resource", a, b)
                if earliest is None or candidate.start_time < earliest.start_time:
                    earliest = candidate

    visited_edge_pairs: set[Tuple[Tuple[str, str], Tuple[str, str]]] = set()
    for edge_key, entries_a in by_edge.items():
        reverse_key = (edge_key[1], edge_key[0])
        entries_b = by_edge.get(reverse_key)
        if not entries_b:
            continue
        pair_key = tuple(sorted((edge_key, reverse_key)))
        if pair_key in visited_edge_pairs:
            continue
        visited_edge_pairs.add(pair_key)
        ordered_a = sorted(entries_a, key=lambda item: item.t_enter)
        ordered_b = sorted(entries_b, key=lambda item: item.t_enter)
        start_b = 0
        for a in ordered_a:
            while start_b < len(ordered_b) and ordered_b[start_b].t_exit <= a.t_enter:
                start_b += 1
            for b in ordered_b[start_b:]:
                if b.t_enter >= a.t_exit:
                    break
                checks += 1
                if a.agent_id == b.agent_id or not _overlaps(a.t_enter, a.t_exit, b.t_enter, b.t_exit):
                    continue
                candidate = _make_conflict("edge", a, b)
                if earliest is None or candidate.start_time < earliest.start_time:
                    earliest = candidate
    return _ConflictSearchResult(conflict=earliest, checks=checks)


def _make_conflict(kind: str, a: ReservationEntry, b: ReservationEntry) -> _Conflict:
    return _Conflict(
        kind=kind,
        agent_a=a.agent_id,
        agent_b=b.agent_id,
        start_time=max(a.t_enter, b.t_enter),
        end_time=min(a.t_exit, b.t_exit),
        resource_id_a=a.resource_id,
        resource_id_b=b.resource_id,
        from_node_a=a.from_node,
        to_node_a=a.to_node,
...
def _format_conflict(conflict: _Conflict) -> str:
    return (
        f"{conflict.kind}:agents={conflict.agent_a},{conflict.agent_b}:"
        f"window={conflict.start_time:.3f}-{conflict.end_time:.3f}:"
        f"resources={conflict.resource_id_a}|{conflict.resource_id_b}"
    )
...
def solve_cbs_mapf(config: PlannerConfig, requirements: Dict[int, List[PathRequirement]]) -> MAPFReservationTable:
    ...
    expanded_nodes = 0
    conflict_checks = 0
    open_set_peak = len(open_set)
    max_expanded = max(1, int(config.mission.mapf_max_expanded_nodes))
    max_conflicts = max(1, int(config.mission.mapf_max_conflicts))
    max_wall_time = max(float(config.mission.mapf_max_wall_time_ms), 0.0) / 1000.0
    ...
    while open_set:
        if expanded_nodes >= max_expanded:
            ...
            budget_reason = "expanded_nodes"
            break
        if resolved_conflicts >= max_conflicts:
            budget_exhausted = True
            budget_reason = "conflicts"
            break
        if max_wall_time > 0.0 and (time.perf_counter() - started) >= max_wall_time:
            budget_exhausted = True
            budget_reason = "wall_time"
            break
        node = heapq.heappop(open_set)
        expanded_nodes += 1
        if node.priority < best_node.priority:
            best_node = node
        search = _find_first_conflict_indexed(node.reservations)
        conflict_checks += search.checks
    ...
    if budget_exhausted and str(config.mission.mapf_fallback or "").strip().lower() == "prioritized_resource_windows":
        fallback_reservations = _prioritized_resource_window_schedule(config, requirements)
        fallback_search = _find_first_conflict_indexed(fallback_reservations)
        conflict_checks += fallback_search.checks
        fallback_makespan = max((entries[-1].t_exit for entries in fallback_reservations.values() if entries), default=0.0)
        unresolved = _collect_conflict_summaries(fallback_reservations)
        return MAPFReservationTable(
            reservations=fallback_reservations,
            conflicts_resolved=resolved_conflicts,
            makespan=fallback_makespan,
            solver_status=f"budget_exhausted:{budget_reason}:fallback",
            expanded_nodes=expanded_nodes,
            open_set_peak=open_set_peak,
            conflict_checks=conflict_checks,
            budget_exhausted=True,
    ...
    final_reservations = best_node.reservations if budget_exhausted else root_reservations
    unresolved = _collect_conflict_summaries(final_reservations)
    final_makespan = max((entries[-1].t_exit for entries in final_reservations.values() if entries), default=makespan)
    return MAPFReservationTable(
        reservations=final_reservations,
        conflicts_resolved=resolved_conflicts,
        makespan=final_makespan,
        solver_status=f"budget_exhausted:{budget_reason}" if budget_exhausted else "open_set_exhausted",
        expanded_nodes=expanded_nodes,
        open_set_peak=open_set_peak,
        conflict_checks=conflict_checks,
...
def _prioritized_resource_window_schedule(
    config: PlannerConfig,
    requirements: Dict[int, List[PathRequirement]],
) -> Dict[int, List[ReservationEntry]]:
    ordered_agents = sorted(
        requirements,
        key=lambda agent_id: (-sum(requirement.duration for requirement in requirements[agent_id]), agent_id),
    )
    scheduled: Dict[int, List[ReservationEntry]] = {}
    committed: List[ReservationEntry] = []
    for agent_id in ordered_agents:
        reservations = _schedule_agent_against_committed(requirements[agent_id], committed)
        scheduled[agent_id] = reservations
        committed.extend(reservations)
    return {agent_id: scheduled.get(agent_id, []) for agent_id in requirements}


def _schedule_agent_against_committed(
    requirements: Sequence[PathRequirement],
    committed: Sequence[ReservationEntry],
        ...
        if requirement.duration < 1e-9:
            ...
            while True:
                ...
                for existing in committed:
                    if not _requirement_conflicts_with_reservation(requirement, t_enter, t_exit, existing):
                        continue
                    t_enter = existing.t_exit
                    shifted = True
                    break
                if not shifted:
                    break
...
def _requirement_conflicts_with_reservation(
    requirement: PathRequirement,
    t_enter: float,
    t_exit: float,
    existing: ReservationEntry,
) -> bool:
    if not _overlaps(t_enter, t_exit, existing.t_enter, existing.t_exit):
        return False
    if requirement.resource_id == existing.resource_id:
        return True
    return requirement.from_node == existing.to_node and requirement.to_node == existing.from_node


def _collect_conflict_summaries(reservations: Dict[int, List[ReservationEntry]], limit: int = 20) -> List[str]:
    summaries: List[str] = []
    scratch = {agent_id: list(entries) for agent_id, entries in reservations.items()}
    for _ in range(limit):
        search = _find_first_conflict_indexed(scratch)
        if search.conflict is None:
            break
        conflict = search.conflict
        summaries.append(_format_conflict(conflict))
        # Remove one conflicting entry in the scratch copy to expose additional
        # conflicts without mutating the returned reservation table.
        entries = scratch.get(conflict.agent_b, [])
        scratch[conflict.agent_b] = [
            entry
            for entry in entries
            if not (
                entry.resource_id == conflict.resource_id_b
                and abs(entry.t_enter - conflict.start_time) <= max(entry.t_exit - entry.t_enter, 1e-9)
            )
        ]
    return summaries

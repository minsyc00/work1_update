...
def run_paper_style_region_tsp_experiment(
    ...
    if path_config.enable_joint_region_candidate_optimization:
        stage_started = time.perf_counter()
    ...
    for agent_id, region_ids in sorted(assignment.agent_regions.items()):
        ...
        tsp_records[agent_id] = {
            ...
            "connector_noncover_repeat_length": float(result.get("connector_noncover_repeat_length", 0.0)),
            "connector_noncover_repeat_penalty": float(result.get("connector_noncover_repeat_penalty", 0.0)),
            "connector_length": float(result.get("connector_length", 0.0)),
            "connector_turn_angle": float(result.get("connector_turn_angle", 0.0)),
...
def _coverage_aware_merge_regions(
    ...
    for _ in range(max_iterations):
        geometric_candidates: List[Tuple[float, int, int, object]] = []
        for first_idx, first in enumerate(current):
            for second_idx in range(first_idx + 1, len(current)):
                ...
                if candidate is None:
                    _increment_object_diagnostic(rejected, reason)
                    continue
                geometric_candidates.append((float(candidate.area), first_idx, second_idx, candidate))
        if not geometric_candidates:
            break

        geometric_candidates.sort(key=lambda item: (-item[0], item[1], item[2]))
        ...
        for _, first_idx, second_idx, candidate in geometric_candidates[:preview_limit]:
            ...
            if not after_preview.get("feasible", False):
                _increment_object_diagnostic(rejected, str(after_preview.get("reason", "no_feasible_pattern")))
                continue
        ...
        for improvement, improvement_ratio, first_idx, second_idx, candidate, before_preview, after_preview in scored[:validate_top_k]:
            ...
            if not validated.get("feasible", False):
                _increment_object_diagnostic(rejected, str(validated.get("reason", "internal_sweep_infeasible")))
                continue
...
def _coverage_merge_candidate_from_group(
    ...
    if full_rectangle and not bbox_collides:
        return (
            DecomposedRegion(
                region_id=f"coverage_merge_region_{serial}",
                bounds=bounds,
                polygon=polygon,
...
def _coverage_merge_before_preview(
    ...
    return {"score": score, "feasible_count": feasible_count, "connector_proxy": connector_proxy}
...
def _coverage_merge_pattern_preview(
    ...
    preview_config = replace(
        ...
        max_entry_exit_patterns_per_region=max(1, min(int(path_config.max_entry_exit_patterns_per_region), preview_limit)),
    )
...
def _optimize_joint_region_candidate_assignment(
    ...
    eval_path_config = replace(path_config, monitor_stages=False)
...
def _evaluate_joint_assignment_solution(
    ...
    objective = (
        ...
        + totals["transition_length"]
        + 0.05 * totals["total_length"]
        + max(report_path_config.global_noncover_repeat_weight, 0.0) * noncover_repeat
        + max(report_path_config.global_cross_agent_overlap_weight, 0.0) * cross_score.overlap_length
        + 10.0 * max(report_path_config.global_cross_agent_overlap_weight, 0.0) * crossing_count
        + max(report_path_config.global_turn_angle_weight, 0.0) * totals["total_turn_angle"]
...
def _refine_global_routes(
        ...
        for agent_id, agent in sorted(agents.items()):
            ...
            for segment in agent.segments:
                if segment.kind == "cover" or len(segment.waypoints) < 2:
                    ...
                    continue
                alternatives = _route_refinement_alternatives(segment, config, path_config, obstacle_field)
                ...
                for candidate_segments, source in alternatives:
                    ...
                    candidate_turn = sum(_segment_heading_variation(item) for item in candidate_segments)
    ...
    if refined_coverage.coverage_fraction + 1e-9 < baseline_coverage:
        for agent_id, original in original_agent_segments.items():
            ...
            agents[agent_id].metrics = _agent_metrics(original, config, obstacle_field)
...
def _route_refinement_score(segments: Sequence[PathSegmentSpec], path_config: PathPlanningConfig) -> float:
    return (
        ...
        * sum(_segment_heading_variation(segment) for segment in segments)
    )
...
def _connector_score_components(
    ...
    connector_turn_angle = sum(_segment_heading_variation(segment) for segment in connector)

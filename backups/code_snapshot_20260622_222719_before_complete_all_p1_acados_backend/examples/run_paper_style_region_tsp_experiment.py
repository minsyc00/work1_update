...
def main() -> None:
    ...
    parser.add_argument("--target-coverage", type=float, default=0.99, help="Target free-space coverage fraction.")
    ...
    config, static_obstacles = load_map_for_planner(
        map_path,
        build_usv_fleet(area_length_x, area_length_y, min_turn_radius=args.rmin or 2.0, count=usv_count),
    )

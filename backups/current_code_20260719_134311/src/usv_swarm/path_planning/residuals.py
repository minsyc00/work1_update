...
def evaluate_tour_coverage_state(
    ...
    for tour in tours:
        agent_config = config.for_agent(tour.agent_id) if config.agent_profiles else config
        model = RectangularCoverageModel.from_config(agent_config)
        pass_segments = []
        ...
        mark_coverage_passes(state, pass_segments, model, eta_cov=agent_config.footprint.eta_cov)
...
def assign_residual_backfill(
    ...
    padding = max(
        min(
            (config.profile_for_agent(tour.agent_id).coverage_width for tour in tours),
            default=config.footprint.width_wf,
        )
        / 2.0,
        1e-6,
    )
    ...
    for region in residual_regions:
        ...
        estimated_start_times[best_agent.agent_id] += transition_time + _residual_service_time(
            region,
            config,
            best_agent.agent_id,
        )
...
def _transition_time_to_region(tour: SingleUsvTourPlan, region: DecomposedRegion, config: PlannerConfig) -> float:
    ...
    return dubins_shortest_path(start_pose, target_pose, agent_config.fleet.min_turn_radius).total_length / max(
        agent_config.fleet.cruise_speed,
        1e-6,
    )


def _residual_service_time(region: DecomposedRegion, config: PlannerConfig, agent_id: int) -> float:
    ...
    pass_count = max(
        ...
        int(
            math.ceil(
                short_side
                / max(
                    agent_config.footprint.width_wf * (1.0 - agent_config.mission.overlap_ratio),
                    1e-6,
                )
            )
        ),
    )
    return pass_count * long_side / max(agent_config.fleet.cover_speed, 1e-6)

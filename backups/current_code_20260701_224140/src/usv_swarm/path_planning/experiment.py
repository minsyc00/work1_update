...
from .pipeline import (
    _agent_plans_from_tours,
    _annotate_agents_validity,
    _append_residual_backfill,
    _build_diagnostics,
    _solver_summary_from_tours,
)
...
def run_planning_algorithm_experiment(
    ...
    _record_stage(
        ...
        effective_tsp_solver=solver_summary["effective_tsp_solver"],
        total_initial_objective=sum(record.get("initial_metrics", {}).get("objective", 0.0) for record in tsp_records.values()),
        total_aco_initial_objective=sum(
            float(record.get("tsp_solver_metadata", {}).get("aco_initial_objective") or 0.0)
    ...
    _record_stage(
        ...
        total_aco_best_objective=sum(
            float(record.get("tsp_solver_metadata", {}).get("aco_best_objective") or 0.0)
            for record in tsp_records.values()
        ),
        total_aco_accepted_3opt_count=sum(
            int(record.get("tsp_solver_metadata", {}).get("aco_accepted_3opt_count") or 0)
            for record in tsp_records.values()
        ),
    )
    _record_stage(
    ...
    _record_stage(
        ...
        shared_resource_count=int(shared_after["shared_resource_count"]),
        shared_resource_conflict_count=int(shared_before["true_time_conflict_count"]),
        spatial_overlap_reuse_count=int(shared_after["spatial_overlap_reuse_count"]),
        true_time_conflict_count=int(shared_after["true_time_conflict_count"]),

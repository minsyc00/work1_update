...
def apply_lightweight_load_swap(
    ...
    loads = {agent_id: float(assignment.loads.get(agent_id, 0.0)) for agent_id in agent_regions}
    before = _imbalance_ratio(agent_regions, loads)
    swap_count = 0
    for _ in range(max(max_iterations, 0)):
        active_agents = [agent_id for agent_id, regions in agent_regions.items() if regions]
        if len(active_agents) < 2:
            break
        heavy = max(active_agents, key=lambda agent_id: loads.get(agent_id, 0.0))
        light = min(active_agents, key=lambda agent_id: loads.get(agent_id, 0.0))
        current = _imbalance_ratio(agent_regions, loads)
        candidate = _best_swap_region(agent_regions, loads, graph, heavy, light, current)
    ...
    after = _imbalance_ratio(agent_regions, loads)
    ...
    active_loads = [loads[agent_id] for agent_id, regions in agent_regions.items() if regions]
    return BalancedAssignment(
        agent_regions=agent_regions,
        loads=loads,
        connected=connected,
...
def _best_swap_region(
    ...
    for region_id in list(source):
        ...
        trial_imbalance = _imbalance_ratio(trial_regions, trial_loads)
...
def _imbalance_ratio(agent_regions: Dict[int, List[str]], loads: Dict[int, float]) -> float:
    active_loads = [loads.get(agent_id, 0.0) for agent_id, regions in agent_regions.items() if regions]
    if not active_loads:
        return 0.0

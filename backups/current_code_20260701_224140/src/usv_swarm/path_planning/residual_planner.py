...
from .obstacles import clipped_axis_aligned_segments, path_segment_invalid_reasons
...
def append_residual_local_tsp(
    ...
    if large_map_mode:
        residual_obstacle_aware_retry_limit = max(16, min(24, len(backfill.residual_regions) * 4))
    region_cell_counts = {
    ...
    remaining = {region.region_id for region in backfill.residual_regions if patterns.get(region.region_id)}
    while remaining:
        best_choice = None
        for agent_id, tour in sorted(tours.items()):
            for region_id in sorted(remaining):
                for pattern in patterns.get(region_id, []):
                    ...
                    if (
                        (not candidate_segments or not _segments_valid(candidate_segments, config, obstacle_field))
                    ...
                    if gain_per_meter + 1e-9 < max(path_config.residual_min_gain_per_path_meter, 0.0):
...
def _residual_patterns(
    ...
    if max(config.mission.area_length_x, config.mission.area_length_y) >= max(path_config.large_map_size_threshold, 1e-6):
        return {region.region_id: _fallback_residual_patterns(region, config, obstacle_field)[:8] for region in residual_regions}

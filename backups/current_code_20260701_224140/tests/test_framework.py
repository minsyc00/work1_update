...
class FrameworkTests(unittest.TestCase):
    ...
    def test_process_backend_hard_timeout_falls_back_without_blocking_runtime(self) -> None:
        ...
        config.mission.nmpc_solver_backend = "auto"
        ...
        self.assertEqual(runtime.profiler.nmpc_solver_backend_requested, "auto")
        self.assertTrue(all("nmpc hard timeout fallback" in result.safety_status.warnings for result in results.values()))

    def test_nmpc_solver_backend_auto_falls_back_to_casadi_when_acados_unavailable(self) -> None:
        config = build_test_config()
        config.mission.nmpc_solver_backend = "auto"
        plan = plan_global_coverage(config)
        runtime = SwarmRuntime(config, plan)

        self.assertEqual(runtime.nmpc_solver_backend_requested, "auto")
        self.assertEqual(runtime.nmpc_solver_backend_effective, "casadi")
        self.assertFalse(runtime.acados_available)
        self.assertIn("acados", runtime.acados_fallback_reason.lower())

    def test_nmpc_solver_backend_casadi_is_forced_without_acados_probe(self) -> None:
        config = build_test_config()
        config.mission.nmpc_solver_backend = "casadi"
        plan = plan_global_coverage(config)
        runtime = SwarmRuntime(config, plan)

        self.assertEqual(runtime.nmpc_solver_backend_requested, "casadi")
        self.assertEqual(runtime.nmpc_solver_backend_effective, "casadi")
        self.assertEqual(runtime.acados_fallback_reason, "")

    def test_nmpc_solver_backend_acados_mode_reports_unavailable_clearly(self) -> None:
        config = build_test_config()
        config.mission.nmpc_solver_backend = "acados"
        plan = plan_global_coverage(config)

        with self.assertRaises(RuntimeError) as raised:
            SwarmRuntime(config, plan)

        self.assertIn("acados", str(raised.exception).lower())

    def test_nmpc_solver_backend_invalid_value_is_rejected(self) -> None:
        config = build_test_config()
        config.mission.nmpc_solver_backend = "bad_backend"
        plan = plan_global_coverage(config)

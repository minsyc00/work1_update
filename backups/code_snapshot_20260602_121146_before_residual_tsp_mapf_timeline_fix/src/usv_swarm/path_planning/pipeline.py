...
def _append_residual_backfill(
    ...
    if not config.mission.residual_enable or not path_config.enable_residual_backfill:
        return 0
    appended_total = 0
    for cycle in range(max(path_config.residual_backfill_cycles, 1)):
        coverage_state = evaluate_tour_coverage_state(
            config,
            list(tours.values()),
            resolution=path_config.residual_resolution,
        ...
        patterns = generate_all_region_patterns(backfill.residual_regions, config, path_config, obstacle_field=obstacle_field)
        cycle_appended = 0
        for agent_id, region_ids in backfill.agent_regions.items():
            tour = tours.get(agent_id)
            if tour is None:
                continue
            for region_id in region_ids:
                candidates = [candidate for candidate in patterns.get(region_id, []) if candidate.passes]
                if not candidates:
                    continue
                current_pose = _tour_end_pose(tour, config)
                current_time = _tour_end_time(tour)
                pattern = min(candidates, key=lambda item: _transition_time(current_pose, item.entry_pose, config) + item.estimated_time)
                new_segments = _build_residual_pattern_segments(
                    agent_id,
                    f"{region_id}_cycle_{cycle}",
                    pattern,
                    current_pose,
                    current_time,
                    config,
                    path_config,
                    obstacle_field,
                    start_serial=len(tour.segments),
                )
                if not new_segments:
                    continue
                if any(
                    path_segment_invalid_reasons(segment, config, obstacle_field)
                    or segment.metadata.get("kinematic_feasible") == "false"
                    for segment in new_segments
                ):
                    tour.diagnostics["residual_backfill_skipped_invalid"] = "true"
                    continue
                selected_id = f"{region_id}_cycle_{cycle}"

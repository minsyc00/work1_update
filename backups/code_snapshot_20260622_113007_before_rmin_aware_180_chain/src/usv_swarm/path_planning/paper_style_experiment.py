...
def run_paper_style_region_tsp_experiment(
    ...
    stage_started = time.perf_counter()
    sweep_paths, feasible_patterns, infeasible_regions = _build_region_sweep_paths(
        raw_patterns,
        config,
        path_config,
        obstacle_field,
    ...
    finish_stage("load_balancing_assignment", stage_started, agent_count=len(assignment.agent_regions))
    stage_started = time.perf_counter()
    ...
    coverage_complete = (
        ...
        and not skipped_region_ids
    )
    report["coverage_status"] = "complete" if coverage_complete else "incomplete"
...
def _apply_large_map_defaults(path_config: PathPlanningConfig, config: PlannerConfig) -> PathPlanningConfig:
    ...
    return replace(
        ...
        max_prefiltered_variants_per_pattern=max(int(path_config.max_prefiltered_variants_per_pattern), 4),
        max_entry_exit_patterns_per_region=min(max(int(path_config.max_entry_exit_patterns_per_region), 1), 4),
        region_tsp_beam_width=min(max(int(path_config.region_tsp_beam_width), 1), 4),
        region_tsp_branch_limit=min(max(int(path_config.region_tsp_branch_limit), 1), 8),
    ...
    for region_id, candidates in sorted(raw_patterns.items()):
        ...
        for pattern in candidates:
            ...
            for variant in variants:
                ...
                valid, reason = _validate_internal_sweep(
                    variant,
                    config,
                    path_config,
                    obstacle_field,
...
def _prefilter_region_patterns(
    ...
    if len(candidates) <= limit:
        return list(candidates)
    ranked = sorted(candidates, key=lambda item: (_light_pattern_score(item, config), item.pattern_id))
    ...
    for axis in sorted({pattern.scan_axis for pattern in ranked}):
        ...
        if axis_candidates:
            best = max(
                axis_candidates,
                key=lambda item: (_estimated_pattern_coverage_fraction(item, config), -_light_pattern_score(item, config), item.pattern_id),
            )
...
def _prefilter_pattern_variants(
    ...
    if len(variants) <= limit:
        return list(variants)
    ranked = sorted(variants, key=lambda item: (_light_pattern_score(item, config), item.pattern_id))
    selected: List[RegionCoveragePattern] = []
    seen_keys = set()
...
def _light_pattern_score(pattern: RegionCoveragePattern, config: PlannerConfig) -> float:
...
def _split_pattern_into_open_chains(
    ...
    def close_current(right_reason: str = "") -> None:
        ...
        chain = _make_open_sweep_chain(
            ...
            obstacle_field=obstacle_field,
            start_time=start_time,
            segment_prefix=f"{segment_prefix}_open_chain_{chain_index}",
...
def _solve_agent_region_tsp(
    ...
    if _large_map_mode_enabled(config, path_config) and len(initial_order) > _large_map_aco_region_limit(path_config):
    ...
    for _ in range(len(initial_order)):
        ...
        for state in beam:
            ...
            if use_coverage_priority:
                candidate_window = branch_limit
            else:
                candidate_window = max(branch_limit, 40) if not state["segments"] else branch_limit
            candidate_slice = ordered_candidates[:candidate_window]
...
def _solve_agent_region_tsp_large_map_greedy(
    ...
    while remaining:
            ...
            for pattern in candidates[:2]:
                ...
                ordered_candidates.append(
                    (
                        ...
                        + _metadata_float(pattern.metadata, "internal_repeat_penalty", 0.0)
                        + path_config.coverage_priority_weight * coverage_deficit
        ...
        candidate_window = max(8, int(path_config.region_tsp_branch_limit))
        for candidate_idx, (_, _, region_id, candidate_pattern) in enumerate(ordered_candidates[:candidate_window]):
            candidate_attempt_count += 1
            ...
            if connector is None and candidate_idx < 2:
            ...
            lookahead_reachable = _large_map_lookahead_reachable_count(
                agent_id=agent_id,
                serial=serial + len(candidate_segments),
                current_pose=candidate_pattern.exit_pose,
                current_time=_segment_end_time(sweep_segments[-1]),
            ...
            lookahead_probe_limit = max(4, min(8, int(path_config.region_tsp_branch_limit)))
                ...
                or [
                    {
                        "agent_id": agent_id,
                        "region_id": region_id,
                        "reason": "no_valid_large_map_greedy_candidate",
                    }
                    for region_id in remaining
                ]
            )
        ...
        feasible_choices.sort(
            key=lambda item: (
                min(0, -int(item["lookahead_reachable"])),
                float(item["score"]),
                str(item["region_id"]),
        ...
        sweep_segments = chosen["sweep_segments"]
        region_id = candidate_pattern.region_id
        candidate_segments = chosen["candidate_segments"]
        repeat_score = chosen["repeat_score"]
...
def _large_map_lookahead_reachable_count(
    ...
    reachable = 0
    probe_limit = max(4, min(8, int(path_config.region_tsp_branch_limit)))
    for region_id in ordered[:probe_limit]:
        region_reachable = False
        for pattern in sorted(patterns.get(region_id, []), key=lambda item: (_pattern_sort_key(item, config, path_config), item.pattern_id))[:2]:
...
def _append_skipped_region_recovery(
    ...
    if not _large_map_mode_enabled(config, path_config):
        return {"enabled": False, "recovered_count": 0, "failed_count": 0}
    ...
    failed: Dict[str, str] = {}
    connector_cache: Dict[Tuple[float, float, float, float, float, float, float, str, bool], Tuple[List[PathSegmentSpec] | None, List[Dict[str, object]]]] = {}
    max_attempts = min(len(skipped), max(3, min(6, int(path_config.max_residual_backfill_regions))))
    for _ in range(max_attempts):
        best_choice = None
        active_skipped = sorted(
            [region_id for region_id in skipped if region_id not in recovered],
            key=lambda region_id: _skipped_region_tail_distance(region_id, feasible_patterns, agents, config),
        )[:12]
        for region_id in active_skipped:
            ...
            for agent_id, agent in sorted(agents.items()):
                ...
                for pattern in sorted(feasible_patterns.get(region_id, []), key=lambda item: (_pattern_sort_key(item, config, path_config), item.pattern_id))[:3]:
                    ...
                    if connector is None and len(connector_cache) < 80:
                        rejections = []
                        connector = _build_region_connector_cached(
                            agent_id=agent_id,
                            serial=serial,
                    ...
                    if connector is None:
                        if rejections:
                            failed.setdefault(region_id, str(rejections[-1].get("reason", "connector_failed")))
                        continue
                    connector_end_time = _segment_end_time(connector[-1]) if connector else current_time
                    sweep_segments, reason = _build_internal_sweep_segments(
                        pattern,
                    ...
                    key = (score, agent_id, region_id, pattern.pattern_id)
        ...
        if record is not None:
            ...
            record["skipped_regions"] = [item for item in record.get("skipped_regions", []) if item != region_id]

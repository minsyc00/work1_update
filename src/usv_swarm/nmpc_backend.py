...
from .nmpc import NMPCResult, create_nmpc_controller
...
_WORKER_CONTROLLERS: Dict[int, object] = {}
...
def _solve_request_in_worker(request: NMPCSolveRequest) -> NMPCBackendSolveResult:
    ...
    if controller is None:
        controller = create_nmpc_controller(
    ...
    return NMPCBackendSolveResult(
        ...
        solver_backend_effective=str(getattr(controller, "solver_backend_effective", "")),
        acados_available=bool(getattr(controller, "acados_available", False)),
        acados_fallback_reason=str(getattr(controller, "acados_fallback_reason", "")),
    )


ProcessCasadiNMPCBackend = ProcessNMPCBackend

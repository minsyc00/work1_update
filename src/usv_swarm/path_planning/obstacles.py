...
def normalize_obstacle_field(
    ...
    # The global planner inflates static obstacles for centerline planning.  The
    # swept coverage/vehicle footprint is handled later by footprint collision
...
def _normalized_obstacle(obstacle: StaticObstacle, inflation: float, path_config: PathPlanningConfig) -> StaticObstacle:
    ...
    if kind == "rectangle":
        ...
        polygon = _inflate_polygon_offset(obstacle.polygon, inflation)
    else:
...
def _inflate_polygon_offset(polygon: Sequence[Point], margin: float) -> List[Point]:
    if margin <= 1e-12:
        return list(polygon)
    points = _ensure_ccw(polygon)
    count = len(points)
    if count < 3:
        return list(points)
    ...
    for idx, start in enumerate(points):
        ...
        offset_edges.append(
            (
                (start[0] + margin * nx, start[1] + margin * ny),
                (end[0] + margin * nx, end[1] + margin * ny),
            )
        )
    inflated: List[Point] = []
    for idx in range(count):
        previous_edge = offset_edges[(idx - 1) % count]
        current_edge = offset_edges[idx]
        intersection = _line_intersection(previous_edge[0], previous_edge[1], current_edge[0], current_edge[1])
        if intersection is None:
            prev_normal = _edge_outward_normal(points[(idx - 1) % count], points[idx])
            next_normal = _edge_outward_normal(points[idx], points[(idx + 1) % count])
            nx = prev_normal[0] + next_normal[0]
            ny = prev_normal[1] + next_normal[1]
            norm = math.hypot(nx, ny)
            if norm <= 1e-9:
                inflated.append(points[idx])
            else:
                inflated.append((points[idx][0] + margin * nx / norm, points[idx][1] + margin * ny / norm))
        else:
            inflated.append(intersection)
    return _drop_near_duplicate_polygon_points(inflated)


def _edge_outward_normal(start: Point, end: Point) -> Point:
    dx = end[0] - start[0]
    dy = end[1] - start[1]
    length = math.hypot(dx, dy)
    if length <= 1e-9:
...
def _drop_near_duplicate_polygon_points(points: Sequence[Point]) -> List[Point]:
    deduped: List[Point] = []
    for point in points:
        if not deduped or math.hypot(point[0] - deduped[-1][0], point[1] - deduped[-1][1]) > 1e-9:

...
本版本已按当前区域分解、倾斜往复扫描、区域间连接和 residual 补扫算法做适配：障碍物保持圆形、椭圆、凸多边形、浅凹多边形和 S 型障碍等多形态，但数量控制在 7 个代表性障碍，并在障碍之间保留宽通道，避免深凹口、密集小礁石和窄死胡同导致区域分解碎片化或连接验证长时间搜索。
...
| `s_curve_shallow_polygon_w_01` | polygon/shallow-concave-S | vertices=`(44,138) -> (80,146) -> (64,156) -> (50,162) -> (80,174) -> (66,188) -> (42,184) -> (56,172) -> (38,164)` |
| `north_convex_island_01` | polygon/convex | vertices=`(104,158) -> (144,152) -> (176,172) -> (156,196) -> (116,190) -> (96,170)` |

## S 型障碍说明

`s_curve_shallow_polygon_w_01` 使用单个浅凹多边形表达短 S 型障碍。该障碍不是长距离屏障，凹入幅度较浅，主要用于测试倾斜边界、浅凹边界和 S 型障碍附近的区域分解，同时避免把西北侧自由水域切成过多小碎片。

## 适配当前算法的设计说明

- 保留一个浅凹多边形 `shallow_concave_island_e_01`，凹口较宽且凹入较浅，不形成 U 形死胡同。
- S 型障碍使用单个低顶点数浅凹多边形表达，不使用长凹多边形切开地图。
...
```text
outputs/static_obstacle_map_200x200_mixed_obstacles_s_polygon_usv4_footprint4x2_rmin2/
```
